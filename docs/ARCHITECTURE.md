...
`_migrations` table. This includes `env_defaults`. Migration `20260915181257`
creates the table on fresh databases and adopts it when it already exists,
preserving its data. Any existing `_dashboard_migrations` table is retained as
historical bookkeeping and requires no updates. Run the API db-migrator before
deploying updated services to a database that lacks the table; no separate
dashboard migration runner is needed. Rolling back this migration retains the
table and its data.
